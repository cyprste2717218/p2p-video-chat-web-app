...
A root `.env` (copied from `.env.example`) is required and is shared by both the frontend and backend containers — see README's Environment Variables section for the full variable list (`JWT_SECRET`, `REFRESH_TOKEN_SECRET`, `DB_*`, `NGROK_HOST`, `LOCAL`, `NODE_ENV`).
...
- `openapi.yaml` — API schema reference; **may drift from actual implementation**, don't treat as ground truth without checking the controller.
...
- `DELETE /call/:callID/leave` is a stub — do not assume leave-call works end-to-end.
